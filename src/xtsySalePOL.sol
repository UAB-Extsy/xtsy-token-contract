...
/**
 * @title xtsySalePOL
 * @dev Simplified Polygon presale contract - only tracks purchases, no token allocation or vesting
 * @notice Presale Price: $0.025 | Public Price: $0.10+ (dynamic)
 * @notice Backend handles all token distribution on Ethereum mainnet
 * @notice Accepts POL, USDT, and USDC on Polygon network
 */
contract xtsySalePOL is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public totalPolRaised;      // Total POL collected
    ...
    event FundsWithdrawn(uint256 usdtAmount, uint256 usdcAmount, uint256 polAmount);
    ...
    function setPolUsdPriceFeed(address _priceFeed) external onlyOwner {
        if (_priceFeed == address(0)) revert ZeroAddress();
        polUsdPriceFeed = AggregatorV3Interface(_priceFeed);
    }
    ...
    function buyTokensWithPOL(
        ...
        _buyTokensWithPOL(address(0), nonce, signature);
    }
    
    function buyTokensWithPOLAndReferral(
        address referrer,
        uint256 nonce,
        bytes calldata signature
    ) external payable nonReentrant whenNotPaused {
        _buyTokensWithPOL(referrer, nonce, signature);
    }
    ...
    function _buyTokensWithPOL(
        ...
        // Get POL price in USD (with 8 decimals from Chainlink)
        uint256 polPriceUsd = getLatestPOLPrice();
        
        // Convert POL amount to USD (6 decimals to match USDT/USDC)
        // msg.value is in wei (18 decimals), polPriceUsd is 8 decimals
        // Result: (wei * price_8_decimals) / 10^20 = USD_6_decimals
        uint256 usdAmount = (msg.value * polPriceUsd) / (10**20);
        
        // Handle referral - give 5% of POL to referrer
        if (referralEnabled && referrer != address(0) && referrer != msg.sender) {
            uint256 referrerPolBonus = (msg.value * referralBonusPercent) / 1000; // 5% of POL payment
            if (referrerPolBonus > 0) {
                (bool success, ) = referrer.call{value: referrerPolBonus}("");
                require(success, "POL referral transfer failed");
            }
        }
        
        _recordPurchase(msg.sender, usdAmount, referrer, "POL");
    ...
    function _recordPurchase(address buyer, uint256 usdAmount, address referrer, string memory paymentMethod) internal {
        ...
        // Emit cross-chain event for backend processing (Polygon Chain ID = 137)
        emit CrossChainPurchase(buyer, usdAmount, expectedTokens, currentPhase, referrer, paymentMethod, 137);
    }
    ...
    function getLatestPOLPrice() public view returns (uint256) {
        if (address(polUsdPriceFeed) == address(0)) revert ZeroAddress();
    ...
    function getLatestPOLPriceUnchecked() public view returns (uint256) {
        if (address(polUsdPriceFeed) == address(0)) return 0;
        
        try polUsdPriceFeed.latestRoundData() returns (uint80, int256 price, uint256, uint256, uint80) {
    ...
    function debugPOLCalculation(uint256 polAmount) external view returns (
        uint256 polPriceUsd,
        uint256 currentRate,
        uint256 polInUsd, 
        uint256 tokensFromPOL,
        bool priceFeedExists,
        bool polAmountPositive,
        bool currentRatePositive
    ) {
        currentRate = getCurrentRate();
        priceFeedExists = address(polUsdPriceFeed) != address(0);
        polAmountPositive = polAmount > 0;
        currentRatePositive = currentRate > 0;
        
        if (polAmount > 0 && currentRate > 0 && address(polUsdPriceFeed) != address(0)) {
            try this.getLatestPOLPrice() returns (uint256 price) {
                polPriceUsd = price;
                polInUsd = (polAmount * polPriceUsd) / (10**20);
                tokensFromPOL = (polInUsd * 10**18) / currentRate;
            } catch {
                polPriceUsd = 999999999; // Error marker
            }
        }
    }
    ...
    function withdrawFunds() external onlyOwner {
        ...
        if (polBalance > 0) {
            (bool success, ) = payable(owner()).call{value: polBalance}("");
            require(success, "POL withdrawal failed");
        }
        
        emit FundsWithdrawn(usdtBalance, usdcBalance, polBalance);
    }
    ...
    function getTokenAmountForPayment(uint256 usdAmount, uint256 polAmount) external view returns (uint256 tokensFromUSD, uint256 tokensFromPOL) {
        uint256 currentRate = getCurrentRate();
        ...
        // Calculate tokens from POL amount
        if (polAmount > 0 && currentRate > 0 && address(polUsdPriceFeed) != address(0)) {
            uint256 polPriceUsd = getLatestPOLPriceUnchecked();
            if (polPriceUsd > 0) {
                // Convert POL to USD (6 decimals)
                uint256 polInUsd = (polAmount * polPriceUsd) / (10**20);
                tokensFromPOL = (polInUsd * 10**18) / currentRate;
            }
        }
        
        return (tokensFromUSD, tokensFromPOL);
    }
