...
contract POLSaleTest is Test {
    ...
    function setUp() public {
        ...
        // Deploy POL presale (without price feed for simplicity)
    ...
    function testBasicSetup() public {
        // Check initial stats - POL contract only tracks volume, not caps
    ...
    function testUSDTPurchase() public {
        ...
        // Check user purchase data (POL contract only tracks spending)
        (xtsySalePOL.UserPurchase memory purchase,) = presale.getUserInfo(alice);
    ...
    function testReferralPurchase() public {
        ...
        // Check referrer info
        (, xtsySalePOL.ReferralInfo memory refInfo) = presale.getUserInfo(bob);
    ...
    function testContractStats() public {
        ...
        (
            ...
            uint256 totalPublicVolume,
            uint256 totalUsdtRaised,
            uint256 totalUsdcRaised,
            uint256 totalPolRaised,
            xtsySalePOL.SalePhase currentPhase
        ) = presale.getContractStats();
        
        assertEq(totalPresaleVolume, purchaseAmount);
        assertEq(totalPublicVolume, 0);
        assertEq(totalUsdtRaised, purchaseAmount);
        assertEq(totalUsdcRaised, 0);
        assertEq(totalPolRaised, 0);
        assertEq(uint8(currentPhase), uint8(xtsySalePOL.SalePhase.PresaleWhitelist));
    }
}